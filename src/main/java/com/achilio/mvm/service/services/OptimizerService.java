...
public class OptimizerService {
  ...
  @Transactional
  // TODO: Refacto
  public Optimization optimizeDataset(final String projectId, final String datasetName)
      throws Exception {
    LOGGER.info("New optimization...");
    FetchedDataset dataset = fetcherService.fetchDataset(projectId, datasetName);
    Optimization optimization =
        new Optimization(projectId, dataset.getLocation(), datasetName, true);
    ...
    for (FieldSet fieldSet : optimized) {
      ...
      OptimizationResult result =
          new OptimizationResult(
              fieldSet.getDataset(), fieldSet.getTable(), optimization, statement);
      results.add(result);
    ...
    publisherService.publishOptimization(
        optimization, results, authentication.getCredentials().getAccessToken().getTokenValue());
