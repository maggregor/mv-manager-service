...
public abstract class FieldSetExtractTest {
	...
	private static final String[][] SIMPLE_TABLE_COLUMNS = new String[][]{
			{"col1", ZetaSQLType.TypeKind.TYPE_STRING.name()}, {"col2",
			ZetaSQLType.TypeKind.TYPE_STRING.name()},
			{"col3", ZetaSQLType.TypeKind.TYPE_INT64.name()}, {"col4",
			ZetaSQLType.TypeKind.TYPE_INT64.name()}, {"ts",
			ZetaSQLType.TypeKind.TYPE_TIMESTAMP.name()}};
	...
	public void before() {
		...
		tables.add(createFetchedTable("myproject.mydataset.mytable",
	...
	public void whereClauseAndAggregateInSelect() {
		final String query = "SELECT SUM(col3) FROM mydataset.mytable WHERE " +
				"col3 < 5";
		assertContainsFields(query, new ReferenceField("col3"),
				new AggregateField("SUM(col3)"));
	}

	@Test
	public void whereClauseSingleReference() {
		final String query = "SELECT 'a' FROM mydataset.mytable WHERE col1 =" +
				" " +
				"'a'";
	...
	public void whereClauseMultipleReferences() {
		final String query = "SELECT 'xxx' FROM mydataset.mytable WHERE col1 " +
				"=" +
				" 'xxx' AND col2 = 'yyy'";
		assertContainsFields(query, new ReferenceField("col1"),
				new ReferenceField("col2"));
	...
	public void discoverTablePaths() {
		...
		fetchedQuery = FetchedQueryFactory.createFetchedQuery("SELECT 'xxx' " +
		...
		fetchedQuery = FetchedQueryFactory
				.createFetchedQuery("SELECT COUNT(*) FROM `achilio-dev" +
						".nyc_trips.mvm_123456`");
		extractor.discoverTablePath(fetchedQuery);
	...
	public void groupByMultipleReferences() {
		final String query = "SELECT col1, col2, col3 FROM mydataset.mytable" +
				" " +
	...
	public void simpleSubQueryReference() {
		String query = "SELECT col1 FROM ( SELECT col1 FROM mydataset" +
				".mytable" +
				" )";
		assertExpectedFieldSet(query, new ReferenceField("col1"));
	}
	...
	public void simpleAliasSubQueryReference() {
		final String query = "SELECT myalias FROM ( SELECT col1 as myalias " +
				"FROM mydataset.mytable )";
		assertExpectedFieldSet(query, new ReferenceField("col1"));
	}
	...
	public void extractAggregatesAndReferences() {
		...
		query = "SELECT col1, MIN(col3) FROM mydataset.mytable WHERE col1 = " +
				"'xxx' GROUP BY col1";
		assertContainsFields(query, new ReferenceField("col1"),
				new AggregateField("MIN(col3)"));
		query = "SELECT col1, MIN(col3) FROM mydataset.mytable WHERE col2 = " +
				"'xxx' GROUP BY col1";
		assertContainsFields(query, new ReferenceField("col1"),
				new ReferenceField("col2"),
				new AggregateField("MIN(col3)"));
	...
	public void notExtractAStarSelect() {
		...
		assertZeroFields(query);
		query = "SELECT col3, col4 FROM mydataset.mytable WHERE col1 = 'xxx'" +
				" " +
				"AND col2 = 'xxx' GROUP BY col3, col4";
		assertZeroFields(query);
		query = "SELECT col1, col2, col3, col4 FROM mydataset.mytable GROUP " +
				"BY" +
				" col1, col2, col3, col4";
		assertZeroFields(query);
		query = "SELECT * FROM (SELECT * FROM mydataset.mytable)";
		assertZeroFields(query);
		query = "SELECT * FROM (SELECT * FROM (SELECT col1, col2, col3, col4" +
				" " +
				"FROM mydataset.mytable))";
		assertZeroFields(query);
	}
	...
	public void extractExpression() {
		...
		query = "SELECT IF(col3 < 4, 'bonjour', 'aurevoir') FROM mydataset" +
				".mytable";
		assertExpectedFieldSet(query, new ExpressionField("IF(col3 < 4, " +
				"\"bonjour\", \"aurevoir\")"));
		query = "SELECT CASE WHEN col1 = 'x' THEN 'a' ELSE 'b' END FROM " +
				"mydataset.mytable";
		assertExpectedFieldSet(query, new ExpressionField("CASE WHEN (col1 =" +
				" " +
				"(\"x\")) THEN (\"a\") ELSE (\"b\") END"));
	}
	...
	public void extractTablesWithProject() {
		final String query = "SELECT col1 FROM mydataset.mytable GROUP BY " +
				"col1";
		assertExpectedFieldSet(query, new ReferenceField("col1"));
	}
	...
	public void registerFetchedTable() {
		...
		final FetchedTable fetchedTable = new DefaultFetchedTable("myproject",
				"myotherdataset", "mytable", columns);
	...
	public void aliasOnFunctionsShouldNotBeExtracted() {
		String query = "SELECT col1 as myalias, COUNT(*) as count FROM " +
				"mydataset.mytable GROUP BY myalias";
		assertExpectedFieldSet(query, new ReferenceField("col1"),
	...
	public void aliasOnFunctionsInGroupByShouldNotBeExtracted() {
		String query = "SELECT col1, CONCAT(col1, col2) as myalias, col3 FROM" +
				" " +
				"mydataset.mytable GROUP BY col1, myalias, col3";
		assertExpectedFieldSet(query, new ReferenceField("col1"),
				new ReferenceField("col3"),
	...
	public void noUnderscoreTimestamp() {
		String query = "SELECT col1 AS col1, TIMESTAMP_TRUNC(ts, DAY) AS " +
				"__timestamp, sum(col3) AS SUM_tip_amount__f2041 FROM " +
				"mydataset.mytable GROUP BY col1, __timestamp ORDER BY " +
	...
	public void assertZeroFields(String query) {
		final FieldSet actual = FieldSetHelper.statementToFieldSet(query,
				extractor);
		Assert.assertEquals(FieldSetFactory.EMPTY_FIELD_SET, actual);
		Assert.assertTrue("Actual FieldSet should be empty",
				actual.fields().isEmpty());
	}
	...
	private void assertContainsFields(String query, Field... fields) {
		...
		for (Field field : expected.fields()) {
			Assert.assertTrue(String.format("One field is missing: %s" +
							".\nActual" +
							" fields: %s", field.name(), actual),
	...
	private void assertExpectedFieldSet(FieldSet expected, FieldSet actual) {
		...
		Assert.assertEquals("Actual FieldSet wasn't expected.", expected,
				actual);
	}
